package com.faishalbadri.hijab.util.pagination;
...
/**
 * Created by faishal on 14/02/18.
 */
    ...
    if (isAllowLoadMore) {
      ...
      if (isUseFooterView()) {
        if (!isFooterView(adapter)) {
          ...
          if (totalItemCount
              < previousTotalItemCount) {//swiprefresh reload result to change listsize ,reset pageindex
...
//                            Log.i(mTag, "****totalItemCount:" + totalItemCount + ",previousTotalItemCount:" + previousTotalItemCount + ",currentpage=startingPageIndex");
          } else if (totalItemCount
              == previousTotalItemCount) {//if load failure or load empty data , we rollback  pageindex
